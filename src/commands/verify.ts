...
export const verifyCommand = new Command('verify')
  ...
  .action(() => {
    console.log(chalk.bold('\n  Memory Unlimited - System Verification\n'));
    ...
    results.push(check('Config file', configExists, configExists ? CONFIG_PATH : 'Nicht gefunden - run `memory-unlimited init`'));
    ...
    if (dbExists) {
      try {
        ...
        const sysKnowledge = searchNodes('Memory Unlimited ist ein', 1);
    ...
    if (failed === 0) {
      ...
      console.log(chalk.bold.red(`  KRITISCH: ${failed} Fehler. Run \`memory-unlimited init\` zuerst.`));
    } else {
